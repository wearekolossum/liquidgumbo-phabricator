...
final class LiquidgumboManiphestCustomFieldClonedBy extends ManiphestCustomField implements PhabricatorStandardCustomFieldInterface
{
    ...
    {
        return 'liquidgumbo:maniphest:clonedBy';
    }

    public function getInverseFieldKey()
    {
        return id(new LiquidgumboManiphestCustomFieldClones())->getFieldKey();
    }
    ...
    {
        return pht('Cloned by');
    }

    public function getFieldDescription()
    {
        return pht('This tasks is cloned by tasks a,b,c etc.');
    }
    ...
    public function applyExternalTransactionEffect(PhabricatorApplicationTransaction $xaction)
    {
        ...
        $transaction->setFieldId(id(new LiquidgumboManiphestCustomFieldClones())->getFieldKey());

        /* Only take action when old value != new value */
        if ($xaction->getOldValue() != $xaction->getNewValue() && PhabricatorEnv::getEnvConfig('liquidgumbo.maniphest.auto-relate-clone') == true) {
